...
func TestTransferTx(t *testing.T) {
	...
	// run n concurrent transfert transactions
	n := 10
	amount := int64(10)

	// To get each channel errors
	errs := make(chan error)
	// To get each channel result
	results := make(chan TransferTxResult)

	for i := 0; i < n; i++ {

		// Get each transaction by name to debug
		go func() {
			// Adding the transactionName txName to the context
